...
public class HandTests
{
    ...
    @Test
    public void testClearContainsNoCards() {

        Hand hand = createHand(card());
        hand.clear();
        assertArrayEquals(emptyArray(), hand.getCards(), "Cleared hand should contain no cards");
    ...
    public void testClearBecomesZeroSize() {
        ...
        Hand hand = createHand(card());
        hand.clear();
        assertEquals(0, hand.size(), "Cleared hand should have zero size");
    ...
    @Test
    public void testGetCardsStreamReturnsEntries() {

        Card card = card();
        Hand hand = createHand(card);
        Stream<Card> cardStream = hand.getCardsStream();
        assertEquals(card, cardStream.findFirst().orElse(null), "Expected card to be present in the stream");

    }

    @Test
    public void testGetCardsStreamOnEmptyHandReturnsEmptyStream() {

        Hand hand = createHand();
        Stream<Card> cardStream = hand.getCardsStream();
        assertEquals(0, cardStream.count(), "Expected stream to have zero length");
    ...
    @Test
    public void testSizeOfEmptyHandReturnsZero() {

        Hand hand = createHand();
        assertEquals(0, hand.size(), "Expected empty hand to have zero size");

    }

    @Test
    public void testSizeOfHandWithCards() {

        Hand hand = createHand(card());
        assertEquals(1, hand.size(), "Expected hand to have size of 1");
